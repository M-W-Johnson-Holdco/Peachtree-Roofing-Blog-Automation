...
6. **OAuth & Permissions** — ensure the bot has scopes used by approval (e.g. `reactions:read`, `chat:write`, `files:read`, channels history as needed).
...
3. **Dockerfile path**: `webhook/Dockerfile` (or use native Python: install `requirements.txt` + `requirements-webhook.txt`, start command below).
...
```bash
pip install -r requirements.txt -r requirements-webhook.txt
PYTHONPATH=src python -m peachtree_blog.slack_webhook.app
```
...
| Path | Role |
|------|------|
| `src/peachtree_blog/slack_webhook/app.py` | FastAPI `/slack/events`, `/health` |
| `src/peachtree_blog/slack_webhook/events.py` | Reaction/message routing |
| `src/peachtree_blog/slack_webhook/github_sync.py` | Pull/push `generated/` and `used_sources.json` |
| `src/peachtree_blog/generated_store.py` | Archive + Slack message index |
