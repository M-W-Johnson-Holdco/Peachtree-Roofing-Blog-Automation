...
from peachtree_blog.slack_webhook.events import process_slack_event_with_git_sync, verify_slack_signature
...
app = FastAPI(title="Peachtree Blog Slack Webhook", version="1.0.0")
...
def main() -> None:
    ...
    uvicorn.run(
        "peachtree_blog.slack_webhook.app:app",
        host="0.0.0.0",
        port=port,
        reload=False,
    )
